...
- **Note Generation**: Codex API (Codex-sonnet)
...
| Step 5 | LLM 笔记生成（Codex） | `backend/services/note_generator.py` | test_ui/pipeline.py Step 4–5 |
...
| `backend/services/note_generator.py` | Codex 笔记生成：4模板×2粒度 + 批注扩写 `generate_annotations()` |
...
| 文件 | 类型 | 用途 |
|------|------|------|
| `backend/test_note_generator_off_slide.py` | pytest | 验证 off-slide 虚拟页面注入逻辑（mock Codex API） |
