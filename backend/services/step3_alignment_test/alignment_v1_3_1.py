"""
Semantic alignment service — Strategy V1.3.1

基于 V1.2（argmax + 后处理平滑），新增单调性惩罚（Monotonicity Penalty）。

改动说明（相对 V1.2）：
─────────────────────────────────────────────────────────────────────────────
唯一改动：在 _smooth_assignments() 的孤立点修正逻辑中，判断平滑方向。

逻辑：
  如果将句段 i 平滑到邻居多数页会导致页码倒退（neighbor_majority < current page），
  则对该次平滑使用更严格的 tolerance（收紧 MONOTONE_PENALTY）。
  正向平滑（向后翻页或保持）沿用原 SMOOTH_TOLERANCE，行为与 V1.2 完全一致。

目的：解决 V1.2 的核心缺陷——"Slide 1 黑洞"效应：
  当老师在讲 Slide 6，但前几句被 Slide 1 的高频宏观词吸走，
  V1.2 会错误地把正确的 Slide 6 向后拖回 Slide 1。
  V1.3.1 通过倒退惩罚保护这类正确的孤立点不被覆盖。

参数：
  SMOOTH_WINDOW     : int   = 2    — 检查前后各 N 句（与 V1.2 相同）
  SMOOTH_TOLERANCE  : float = 0.08 — 正向平滑容忍的相似度损失（与 V1.2 相同）
  MONOTONE_PENALTY  : float = 0.06 — 倒退时额外收紧的 tolerance
                                      (倒退时 effective_tolerance = 0.08 - 0.06 = 0.02，
                                       几乎禁止倒退平滑)
...
STRATEGY_DESCRIPTION = (
    "V1.3.1 — 单遍扫描 + 单调性后处理平滑：在 V1.2 平滑基础上，"
    "对导致页码倒退的平滑操作额外收紧 tolerance（MONOTONE_PENALTY），"
    "防止正确的前进预测被错误的邻居多数派拖回低页码。"
)
...
SMOOTH_TOLERANCE : float = 0.08  # 正向平滑：允许的相似度损失上限
MONOTONE_PENALTY : float = 0.06  # 倒退惩罚：倒退时额外收紧 tolerance
...
def _smooth_assignments(
    ...
    3. [V1.3.1 新增] 判断平滑方向：
       - 若 neighbor_page < best_page_idx[i]（倒退），
         使用 effective_tolerance = tolerance - monotone_penalty（更严格）
       - 否则（前进或保持），使用原 tolerance
    4. 检查 i 在 neighbor_page 上的 cosine 得分是否 >= argmax 得分 - effective_tolerance。
...
def build_page_timeline(
    ...
    """
    Build per-page timeline — V1.3.1 (argmax + monotonicity-penalized smoothing).

    Pipeline:
      1. Embed pages + segments
      2. Argmax cosine assignment
      3. [V1.3.1] Smooth with monotonicity penalty
      4. Apply user anchors as hard overrides
      5. OFF_SLIDE_THRESHOLD filter + assemble output
    """
