"""
Semantic alignment service — Strategy V2 (D-004).
Copy of backend/services/alignment.py at D-004 revision:
  K=3 debounce state machine + whole-PPT off-slide detection + upgrade pass.
"""

STRATEGY_DESCRIPTION = "策略 V2（D-004）：三段式改进版。① K=3 去抖动状态机——连续K段投票同一页才切换；② 整体PPT向量off-slide检测——以全PPT拼接文本embedding为基准；③ 升级通道——off-slide段若与前一已对齐段相似度>0.60则升级回aligned。三次embedding API调用合并为一次批量请求。"
...
def build_page_timeline(
    ...
    # --- K=3 page-switch debounce state machine (without off-slide detection) ---
