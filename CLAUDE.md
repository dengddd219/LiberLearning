...
**MVP-0 Phase 1 已完成**：前后端均已上线。后端 FastAPI 服务完整；前端 React + Vite 已实现全部主要页面和组件，Live 课堂全流程（WebSocket ASR + SSE AI Notes）已打通。PRD 文档在 `LiberStudy-PRD.md`（v0.4）。
...
- **Frontend**: React 19 + Vite + Tailwind CSS v4（注意：**未使用 shadcn/ui**，是原生 Tailwind）
- **Backend**: FastAPI (Python)
- **PPT Parsing & Rendering**: LibreOffice (headless) + PyMuPDF → PDF 渲染（`react-pdf`）
- **Audio Format Conversion**: FFmpeg (WebM/Opus → WAV)
- **Chinese ASR**: Alibaba Cloud ASR API
- **English ASR**: Alibaba Cloud ASR API（fallback: OpenAI Whisper）
- **Semantic Alignment**: OpenAI text-embedding-3-small
- **Note Generation**: Claude API（claude-sonnet）
- **Live 实时**: WebSocket ASR + SSE AI Notes 流式推送
- **DB**: SQLite（`live_data.db` live sessions；`backend/liberstudy.db` 主数据）
- **Deployment**: Cloud deployment（前后端分离），API keys 在服务端环境变量
...
| `backend/services/step3_alignment_test/` | 对齐算法实验版本集合（15个版本 + utils），测试平台 strategy dropdown 用 |
...
| `backend/services/step3_alignment_test/alignment_v1_3_1.py` | V1.3.1 — 单遍扫描 + 改进型平滑 |
| `backend/services/step3_alignment_test/alignment_v1_3_2.py` | V1.3.2 — 单遍扫描 + 增强平滑 |
...
| `backend/services/step3_alignment_test/alignment_v4_1.py` | V4.1 — 两阶段状态机 + 防抖改进 |
| `backend/services/step3_alignment_test/alignment_v5.py` | V5 — 最新实验版本 |
| `backend/services/step3_alignment_test/alignment_v5_1.py` | V5.1 — V5 变体 |
...
| `backend/prompts/ai_frontpage_ask/prompt.md` | AI 首页提问（Live 课堂实时问答） |
...
| `backend/tests/test_db.py` | pytest | 数据库层测试 |
| `backend/tests/test_process_validation.py` | pytest | 流程验证测试 |
...
| `docs/superpowers/specs/2026-04-15-notes-page-presentation-mode-design.md` | NotesPage 演示模式设计规格 |
| `docs/superpowers/specs/2026-04-15-translation-feature-design.md` | 划词翻译功能设计规格 |
| `docs/superpowers/specs/2026-04-16-ai-frontpage-ask-design.md` | AI 首页问答（Live 实时辅助）设计规格 |
| `docs/superpowers/specs/2026-04-16-livepage-design.md` | LivePage 整体设计规格 |
| `docs/superpowers/specs/2026-04-17-progressive-loading-design.md` | 渐进式加载设计规格 |
| `docs/superpowers/specs/2026-04-19-live-asr-realtime-design.md` | Live 实时 ASR 设计规格 |
| `docs/superpowers/specs/2026-04-19-run-log-viewer-design.md` | 运行日志查看器设计规格 |
| `docs/superpowers/specs/2026-04-21-live-granola-replica-design.md` | Live Granola 风格笔记复刻设计规格 |
| `docs/superpowers/specs/2026-04-21-live-product-flow-spec.md` | Live 课堂完整产品流程规格 |
| `docs/superpowers/specs/2026-04-21-lobby-multiselect-delete-design.md` | LobbyPage 多选批量删除设计规格 |
...
| `docs/superpowers/plans/2026-04-13-backend-persistence-and-api-fixes.md` | 后端持久化与 API 修复计划 |
| `docs/superpowers/plans/2026-04-13-lobby-page.md` | LobbyPage 实现计划 |
| `docs/superpowers/plans/2026-04-13-mock-chain-fix.md` | Mock 链修复计划 |
| `docs/superpowers/plans/2026-04-14-button-navigation-wiring.md` | 按钮导航接线计划 |
| `docs/superpowers/plans/2026-04-14-frontend-ui-ux-fixes.md` | 前端 UI/UX 修复计划 |
| `docs/superpowers/plans/2026-04-14-notes-page-redesign.md` | NotesPage 重设计计划 |
| `docs/superpowers/plans/2026-04-15-frontend-demock.md` | 前端去 mock 化计划 |
| `docs/superpowers/plans/2026-04-15-pipeline-runlog-and-fixes.md` | 流水线运行日志与修复计划 |
| `docs/superpowers/plans/2026-04-15-translation-feature.md` | 划词翻译功能实现计划 |
| `docs/superpowers/plans/2026-04-16-ai-frontpage-ask.md` | AI 首页问答实现计划 |
| `docs/superpowers/plans/2026-04-16-canvas-toolbar-features.md` | 画布工具栏功能实现计划 |
| `docs/superpowers/plans/2026-04-16-livepage.md` | LivePage 初始实现计划 |
| `docs/superpowers/plans/2026-04-16-ui-language-switch.md` | UI 语言切换实现计划 |
| `docs/superpowers/plans/2026-04-17-progressive-loading.md` | 渐进式加载实现计划 |
| `docs/superpowers/plans/2026-04-19-live-asr-realtime.md` | Live 实时 ASR 实现计划 |
| `docs/superpowers/plans/2026-04-19-run-log-viewer.md` | 运行日志查看器实现计划 |
| `docs/superpowers/plans/2026-04-20-livepage-refactor-and-rebuild.md` | LivePage 重构与重建计划 |
| `docs/superpowers/plans/2026-04-21-granola-replica-phase1.md` | Granola 风格复刻 Phase 1 计划 |
| `docs/superpowers/plans/2026-04-21-livepage-overhaul.md` | LivePage 大翻新计划 |
...
| `frontend/src/pages/ProcessingPage.tsx` | 处理中等待页：流水线进度显示（注：文件可能已内联或移除，以实际为准） |
...
| 路径 | 内容 |
|------|------|
| `frontend/src/components/bg/LiveBgShell.tsx` | LivePage 背景壳组件 |
| `frontend/src/components/bg/NotesBgShell.tsx` | NotesPage 背景壳组件 |

#### Context & Hooks & Lib

| 路径 | 内容 |
|------|------|
| `frontend/src/context/TabsContext.tsx` | Tab 状态全局 Context |
| `frontend/src/context/TranslationContext.tsx` | 翻译功能全局 Context |
| `frontend/src/hooks/useHighlights.ts` | 高亮标注状态管理 |
| `frontend/src/hooks/useSearch.ts` | 搜索功能 hook |
| `frontend/src/hooks/useSessionEvents.ts` | Live session 事件订阅 hook |
| `frontend/src/hooks/useTextAnnotations.ts` | 文字标注状态管理 |
| `frontend/src/hooks/useTypewriter.ts` | 打字机动画 hook |
| `frontend/src/lib/api.ts` | 后端 API 调用封装 |
| `frontend/src/lib/askDb.ts` | IndexedDB 问答数据访问层 |
| `frontend/src/lib/i18n.ts` | 国际化工具 |
| `frontend/src/lib/idb.ts` | IndexedDB 通用封装（音频 chunk、session draft 持久化）|
| `frontend/src/lib/notesDb.ts` | IndexedDB 笔记数据访问层 |
| `frontend/src/lib/notesTypes.ts` | 笔记相关 TypeScript 类型定义 |
| `frontend/src/lib/notesUtils.ts` | 笔记工具函数 |
| `frontend/src/lib/translation.ts` | 划词翻译逻辑 |
| `frontend/src/lib/xpath.ts` | XPath 工具（用于文本选区定位）|
| `frontend/src/types/session.ts` | Session 相关 TypeScript 类型定义 |
